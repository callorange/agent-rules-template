...
AI 에이전트가 따를 공통 규칙을 프로젝트에 설치하고 업데이트하는 템플릿입니다. 핵심 규칙은 `AGENTS.md`로 제공하며, 언어·프레임워크별 규칙과 스킬은 필요한 작업에 맞춰 적용합니다. 현재 버전은 **2.13.0**입니다.
...
### 고아 디렉터리(Orphan Directory) 처리 옵션

새 버전 템플릿 배포에서 제외된 이전 관리 디렉터리(예: `skills/handoff`)가 로컬 프로젝트에 남아있을 때의 동작과 CLI 옵션입니다.

| 명령 / 실행 환경 | 조건 | 동작 |
| :--- | :--- | :--- |
| `agent-rules` | TTY (대화형 터미널) + 고아 디렉터리 존재 | `[y/N]` 확인 프롬프트 표시 (기본값 N: 보존) |
| `agent-rules` | non-TTY (CI / 자동화 / AI 환경) + 고아 디렉터리 존재 | 질문 없이 안전하게 기본 보존(keep)하며 Project-owned로 전환 |
| `agent-rules --keep-orphans` | 환경 무관 | 질문 없이 고아 디렉터리 및 내부 파일을 Project-owned로 보존 |
| `agent-rules --clean-orphans` | 환경 무관 | 질문 없이 고아 디렉터리와 내부 파일(사용자 추가 파일 포함)을 디스크에서 삭제 |
| `agent-rules -y` / `--yes` | 환경 무관 | 확인 프롬프트에 자동으로 '예'를 선택하며, 고아 디렉터리 정리도 즉시 승인 |

> [!IMPORTANT]
> **`--force`와 `--clean-orphans`의 권한 범위 차이**
> - **`--force`**: 현재 템플릿이 여전히 관리 중인 파일에 로컬 수정(Local modifications)이 있을 때 이를 템플릿 원본 내용으로 덮어쓸 수 있는 권한입니다.
> - **`--clean-orphans`**: 새 템플릿 번들에서 제외된 고아 디렉터리를 디스크에서 영구 삭제·정리하도록 승인하는 권한입니다.
> - 둘은 서로 다른 대상과 목적을 가진 독립적인 권한입니다. `--clean-orphans` 또는 `-y`를 지정하더라도, 고아 디렉터리 외부의 일반 관리 파일이 로컬에서 수정되어 있다면 `--force` 없이는 보호됩니다.

> [!NOTE]
> **논리적 제거 선행 및 커밋 이후 물리 정리(Cleanup)**
> 삭제 승인된 고아 디렉터리는 동기화 과정에서 먼저 원래 경로에서 격리(staging)된 후 트랜잭션이 커밋됩니다. 동기화 커밋 이후 진행되는 임시 저장소 물리 cleanup이 파일시스템 잠금이나 권한 등의 이유로 실패하더라도, 정상 완료된 동기화 결과와 새 baseline은 유지되며 미삭제된 잔여 임시 경로(`.agent-rules-orphan-*`)를 warning으로 안내합니다. 이 임시 경로는 다음 sync에 영향을 주지 않습니다.
