...
- **소통·문서화 및 커밋 언어**: 소통 언어와 문서화 언어는 독립적으로 설정합니다. 별도 지정이 없는 범위에는 사용자의 주 언어를 사용하며, 한 범위에만 지정된 언어를 다른 범위에 전파하지 않습니다. 새 문서, 새 Docstring 및 새 주석은 문서화 언어를 따르고, 기존 내용은 번역이나 언어 통일 요청이 없는 한 기존 언어를 유지합니다. 커밋 설명에는 별도 지정이 없으면 사용자의 주 언어를 사용하며, 구조적 키워드(`feat`, `fix`, `docs`, `refactor` 등), 표준 섹션명 및 기술 고유명사는 영어 원문을 사용할 수 있습니다.
