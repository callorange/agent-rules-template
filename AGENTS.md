...
### 2.1 위험 기반 안전 경계 및 작업 권한 (Safety Boundaries & Authority)
- **자율 실행 (Read-only & Reversible)**: 코드 읽기, 검색, 가역적 로컬 코드 수정, 격리된 테스트 실행은 추가 승인 없이 자율적으로 수행합니다.
- **사전 승인 필수 (High-risk Side-effects)**: 파일 영구 삭제, 외부 전송/비용 발생, 자격 증명 변경, 실제 공유/운영 환경 반영 등 요청되지 않은 고위험 작업은 실행 전 위험도와 변경 내역을 요약 보고하고 사용자 승인을 얻습니다.
- **작업 단위 승인 (Task-level Authorization)**: 사용자가 특정 side-effect 작업이나 외부 패키지 도입을 직접 지시한 경우 해당 의도는 이미 승인된 것으로 간주하며, 동일한 의도의 승인을 다시 묻지 않습니다.
- **보안 및 인코딩**: 자격 증명(API 키, 토큰) 노출 금지(환경 변수 및 `.env` 활용), Windows 작업 시 UTF-8 인코딩 기본 적용.

### 2.2 정량적 기계 검증 및 자가 치유 (Mechanical Validation & Self-Healing)
- **증거 기반 판단**: 하네스는 목적·제약·안전 경계와 검증 가능한 품질을 명확히 하는 데 사용합니다. 에이전트의 주관적 추측을 배제하고 Linter, Type Checker, Test Runner, 빌드 스크립트 등 **정량적 검증 도구**의 관찰 가능한 증거로 완료 여부를 판단합니다.
- **기계적 수렴 우선 (Mechanical Convergence First)**: 매 편집마다 check/fix/check를 반복하는 Check-First 루프를 지양하고, 하나의 논리적 변경 단위가 안정된 시점에 관련 범위를 우선 검증합니다. 서식·단순 구문 오류는 프로젝트 도구가 지원하는 formatter 및 안전한 auto-fix를 먼저 적용해 기계적으로 수렴시킨 후 미해결 오류만 분석합니다.
- **결정적 기계 오류 vs 가설 디버깅**: 도구의 진단 출력만으로 원인과 수정이 결정적인 기계 오류는 가설 루프 없이 직접/자동 수렴으로 신속히 교정하며, 원인이 불명확하거나 복합적인 오류에만 Structured Troubleshooting(원인 가설 → 증거 수집 → 최소 수정)을 적용합니다. 수정 후에는 전체 검증을 반복하지 않고 영향받는 검증만 재수행합니다.
- **문서 동기화 시점**: 매 파일 편집마다 문서를 갱신하는 micro-loop를 지양하고, 논리적 단위의 구현 안정화 후 동일 작업 단위(완료/커밋 전) 내에서 수술적 편집(Surgical Edit)으로 동기화하여 코드와 문서의 최종 정합성을 보장합니다.
...
**Version**: 2.13.1 | **Ratified**: 2026-09-10 | **Architecture**: Modern Planning-First, Risk-Proportional Harness
