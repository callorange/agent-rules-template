...
- **트랜잭션 관리**: 복수의 데이터 변경이 하나의 일관성 경계를 이루어 함께 성공·실패해야 하는 atomic 작업에 트랜잭션을 명시적으로 적용하고 실패 시 Rollback을 보장하십시오. 단순한 복수 CUD라는 이유만으로 불필요한 트랜잭션을 강제하지 않되, 실제 데이터 무결성과 트랜잭션 불변조건(invariant)은 엄격히 유지합니다.
