...
### Fixed
- **고아 디렉터리(Orphan Directory) 삭제 실패 시 트랜잭션 롤백 보장 (`agent-rules`)**:
  - 고아 디렉터리 삭제 단계에서 `OSError` 등 실패가 발생할 경우, 이미 교체된 관리 파일과 `AGENTS.md`, 로컬 메타데이터(`.agent-rules-template.json`)를 이전 버전 baseline으로 완전 롤백하고 임시 이동했던 고아 디렉터리를 원위치로 복구하여 부분 적용 상태가 남지 않도록 보완했습니다.
