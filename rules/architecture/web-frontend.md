...
- **컴포넌트 책임 분리 및 컴포지션**: 컴포넌트는 UI 렌더링과 사용자 인터랙션에 집중하되, 비즈니스 로직(API 호출, 복잡한 상태 처리)의 분리는 실제 복잡성, 컴포넌트 간 재사용 여부, 독립 테스트나 상태 라이프사이클 분리의 실질적 가치, 그리고 프레임워크 권장 방식에 따라 결정합니다.
  분리가 필요한 경우 프레임워크 네이티브 컴포지션 패턴(React: Hook/모듈, Vue: Composable, Svelte: Store/모듈 함수 등) 및 기존 프로젝트 관례를 활용하십시오. 단순한 컴포넌트 로컬 상태나 직접적인 이벤트 처리는 불필요한 추상화 없이 컴포넌트 내부에 둘 수 있으며, 복잡한 도메인 로직과 무제한 섞이지 않도록 균형을 유지합니다.
...
- **지역 상태 우선 (Local State First)**: 전역 상태(Redux, Zustand, Pinia 등)는 앱 전체에서 공유해야 하는 최소한의 데이터에만 사용하고, 컴포넌트 내부 렌더링 상태는 프레임워크의 지역 상태(Local State) 메커니즘으로 격리하십시오.
