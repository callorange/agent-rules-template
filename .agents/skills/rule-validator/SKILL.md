...
### 🎯 검사 항목
* **필수 코어 모듈 존재 여부**: `rules/core/01-base.md` 존재 확인 및 `rules/core/*.md` 동적 모듈 수집
...
1. **상호 충돌 (Conflict) 진단**:
   * `rules/core/01-base.md` (최상위 헌법)의 원칙과 하위 `frameworks/`, `architecture/`, `styles/` 특화 규칙이 정면으로 충돌하는 부분이 있는가? (단, 의도된 하위 모듈의 Override 조항은 제외)
